// app.js — Portfolio frontend logic
// Tailwind CDN removed; styles now served from /dist/tailwind.css (npm run build:css)
...
// NOTE: Clock is already handled by updateTimeAndGreeting() above — no duplicate needed.
...
// Conversation history for multi-turn AI memory (max 10 turns)
const chatHistory = [];
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          history: chatHistory.slice(0, -1) // all turns except the current one
        })
      });
...
document.getElementById('a11y-toggle').addEventListener('click', () => {
  ...
  menu.classList.toggle('flex'); // 'flex' not on HTML element to avoid hidden/flex conflict
});
...
if (contactForm) {
  contactForm.addEventListener('submit', async (e) => {
    ...
    try {
      // Use relative URL so it works in both localhost and production
